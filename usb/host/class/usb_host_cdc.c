                                 ...
                                 void *callbackParam)
{
    ...
    {
#ifdef HOST_ECHO
        PRINTF("Error in allocating transfer!!\n");
#endif
        return kStatus_USB_Error;
    }
                                      ...
                                      void *callbackParam)
{
    ...
    if (classHandle == NULL)
    {
    	PRINTF("USB_HostCdcInterruptRecv param classHandle is NULL!!\n");
        return kStatus_USB_InvalidHandle;
    }

    if (cdcInstance->interruptPipe == NULL)
    {
    	PRINTF("USB_HostCdcInterruptRecv interrupt pipe is NULL!!\n");
        return kStatus_USB_Error;
    }
    ...
    if (USB_HostMallocTransfer(cdcInstance->hostHandle, &transfer) != kStatus_USB_Success)
    {
        ...
        usb_echo("error to get transfer\r\n");
#endif
        PRINTF("In USB_HostCdcInterruptRecv, USB_HostMallocTransfer failed!!\n");
        return kStatus_USB_Error;
    }
...
#endif
        USB_HostFreeTransfer(cdcInstance->hostHandle, transfer);
        PRINTF("In USB_HostCdcInterruptRecv, USB_HostRecv failed!!\n");
        return kStatus_USB_Error;
    }
    return kStatus_USB_Success;
}
